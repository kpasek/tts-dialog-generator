...
class TrainingPlanConfig(BaseModel):
    ...
    #     description="Configuration for pretraining the text encoder tts_model."
    # )
    ...
    alignment: TrainingStageConfig = Field(
        default_factory=TrainingStageConfig,
        description="Configuration for pretraining the text alignment tts_model.",
    )
...
class LossWeightConfig(BaseModel):
    ...
    slm: float = Field(
        ..., description="Weight for speech-language tts_model feature matching loss."
    )
...
class RingformerGeneratorConfig(BaseModel):
    ...
    depth: int = Field(..., description="Number of conformer blocks in tts_model")
...
class SlmConfig(BaseModel):
    ...
    model: str = Field(..., description="Identifier or path for the SLM tts_model.")
    sr: int = Field(..., description="Sampling rate used by the SLM.")


class Config(BaseModel):
    """
    Top-level configuration tts_model that encompasses all settings.
    """
...
class ModelConfig(BaseModel):
    multispeaker: bool = Field(
        ..., description="Indicates if the tts_model supports multispeaker input."
    )
...
def load_from_string(config_str: str) -> Config:
    ...
    Returns:
        Config: Parsed configuration object.
    """
    # Convert JSON string to a dictionary and validate it with the Config tts_model
